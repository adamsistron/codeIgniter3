...
class Examples extends MY_Controller
{
    ...
    public function create_user()
    {
        ...
        $user_data = array(
            'username'   => 'adamsistron',
            'passwd'     => 'ZXasqw12',
            'email'      => 'adamsistron@example.com',
    ...
    public function logout()
    {
        ...
        redirect( base_url('?logout=1'), $redirect_protocol  );
            ...
            if( $this->tokens->match && $this->input->post('email') )
            {
                if( $user_data = $this->examples_model->get_recovery_data( $this->input->post('email') ) )
                {
                    ...
                    else
                    {
                        ...
                        // Set URI of link
                        $link_uri = base_url('examples/recovery_verification/') . $user_data->user_id . '/' . $recovery_code;
            ...
            $(document).ready(function(){
                $(document).on( 'submit', 'form', function(e){
                    $.ajax({
                        type: 'post',
                        cache: false,
                        url: '".base_url('examples/ajax_attempt_login')."',
                        data: {
                        ...
                        success: function(response){
                            ...
                            if(response.status == 1){
                                ...
                                $('#login-link').attr('href','".base_url('examples/logout')."').text('Logout');
